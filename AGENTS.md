...
**Phase 2b — Qdrant backend (done, 2026-06-28):** `QdrantVectorStore` behind the same `VectorStore` interface (`docsmind/index/qdrant_store.py`). Local-path persistence by default (offline, mirrors FAISS save/load under `index_dir/qdrant`), optional server via `qdrant_url` (Docker on DigitalOcean). Cosine distance, Qdrant's built-in HNSW. Selected by new `vector_backend` setting (`faiss` default / `qdrant`). Tests use `:memory:`. Added a `chunks` property to the `VectorStore` ABC (Phase 3's BM25 reads it). `close()` releases the local-path lock.

**Phase 3 — Hybrid retrieval (done, 2026-06-28):** dense + BM25 → RRF → optional cross-encoder rerank. New files: `retrieval/bm25.py` (rank-bm25, rebuilt in-memory from stored chunks), `retrieval/fusion.py` (Reciprocal Rank Fusion, k=60), `retrieval/reranker.py` (`CrossEncoderReranker`, lazy-loaded), and `HybridRetriever` in `retrieval/retriever.py`. Default `retrieval_mode="hybrid"` is **ON** (BM25 fusion needs no model), but `rerank_enabled=False` by default (cross-encoder downloads a model → gated for a DigitalOcean GPU Droplet; model is `ms-marco-MiniLM-L-6-v2`). Dials: `candidate_k=20`, `fusion_k=60`. Existing FAISS index works with hybrid without re-ingest (chunks already persisted); switching to Qdrant needs re-ingest. 28 tests pass (was 17). Deps added: `qdrant-client`, `rank-bm25`.

**Retrieval eval (done, 2026-06-28):** `scripts/retrieval_eval.py` (+ `make eval` / `make digitalocean-eval`, flags `--rerank`, `--chunk-size`) scores dense vs hybrid vs hybrid+rerank on a 15-query labeled set (`data/eval/retrieval_queries.json`, source-level relevance, metrics Hit@1/Hit@3/MRR). Real findings:
...
**Still not built:** answer-quality eval (faithfulness/hallucination — Phase 6, RAGAS/DeepEval; this grades LLM output, not retrieval). No Makefile target yet for running a Dockerized Qdrant *server* on DigitalOcean (local-path mode works without one).

**Next:** Phase 4 (LLM router — which *model* to call) or Phase 6 (answer eval).

## Dev workflow
Canonical git repo lives on the Mac at `/Users/ajmalrasi/docs_mind` — fast file edits, clean history for GitHub, source of truth. Remote execution runs on a DigitalOcean Droplet; GPU-dependent workloads require a GPU Droplet.

`make sync` rsyncs the working tree to `DIGITALOCEAN_HOST:DIGITALOCEAN_DIR` (excludes `.git`, `.venv`, `data/index`, `__pycache__`). The `digitalocean-*` Makefile targets (`digitalocean-install`, `digitalocean-ingest`, `digitalocean-demo`, `digitalocean-serve`, `digitalocean-test`, `digitalocean-eval`) sync and then run over SSH. Set `DIGITALOCEAN_HOST=user@droplet-ip`; the default remote directory is `/home/docsmind/app`.

Why this split: it keeps native file tools and git on the Mac while allowing reproducible remote compute. The DigitalOcean Droplet holds its own virtual environment and runtime index; the Mac remains the source of truth.

## DigitalOcean runtime

Use an Ubuntu Droplet for the API, indexing, Qdrant, and CPU-safe tests. Use a
GPU Droplet for vLLM, CUDA benchmarks, or cross-encoder reranking. Connect with
SSH keys, restrict inbound traffic with a cloud firewall, and keep the project
under `/home/docsmind/app`. Do not place a private IP address or SSH credential
in the repository.
...
**How DocsMind bridges to it:** DocsMind is currently the retrieval half (FAISS/hybrid/rerank) — supporting evidence at best. To make entry-gate bullets #1 and #3 real, deploy a hardware-appropriate 7–12B quantized model on a DigitalOcean GPU Droplet:
...
The user is increasingly asked in interviews **"how did you deploy a model in production?"** Current DocsMind story is only half an answer (managed Anthropic API plus a development model endpoint, not yet a hardened scalable production service). Closing this gap is a top priority — it is the core skill for the Auric target role above.

**Assets:** AWS account with **$100 free credit** (created ~2026-06-28) and DigitalOcean as the remote application/model-serving environment. Select a GPU Droplet whose VRAM fits the chosen quantized model and record the exact hardware in benchmark results.
...
**Allocation:**
- **DigitalOcean GPU Droplet:** serve an open model with **vLLM** behind an OpenAI-compatible streaming endpoint; benchmark TTFT, throughput, GPU utilisation, and hourly cost. This is the missing "self-host and serve" skill.
...
**The interview payoff to aim for:** “I deployed an open model with vLLM on a DigitalOcean GPU Droplet and compared it with a managed model endpoint. I measured cost per token, TTFT, throughput, failure behaviour, and GPU utilisation.”
